...
public async Task<ActionResult<IEnumerable<CycleDto>>> GetCycles()
{
    ...
    return cycles;
        ...
        public async Task<ActionResult<CycleDto>> CreateCycle(CycleCreateDto dto)
        {
            ...
            var farm = await _context.Farms.FindAsync(cycle.FarmId);
            var barn = await _context.Barns.FindAsync(cycle.BarnId);

            var resultDto = new CycleDto
            {
                Id = cycle.Id,
                Name = cycle.Name,
                FarmId = farm!.Id,
            ...
            return CreatedAtAction(nameof(GetCycle), new { id = cycle.Id }, resultDto);
        }
